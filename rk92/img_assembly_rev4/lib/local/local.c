...
int poll_packet(serial_connection_t *serial, long per_poll_delay_ns, size_t max_poll_count, transfer_t *return_packet) {
    assert(serial);
    assert(per_poll_delay_ns>=0);
    assert(max_poll_count>=0);
    assert(return_packet);

    struct timespec remaining = {0};
    struct timespec request = {0,per_poll_delay_ns};

    int bytes_available = 0;

    for(size_t poll_iter=0; poll_iter<max_poll_count; poll_iter++) {
        bytes_available = 0;
        if(ioctl(serial->serial_port,FIONREAD,&bytes_available)<0) {
            printf("(poll_packet): %d: %s\n",errno, strerror(errno));
            nanosleep(&request,&remaining);
        } else if(bytes_available==0) {
            nanosleep(&request,&remaining);
        } else {
            if(serial_receive(serial)>0) {
                if(parse_buffer(return_packet)>0) {
                    return return_packet->id;
                } else {
                    nanosleep(&request,&remaining);
                }
            } else {
...
struct ack_packet create_pack() {
    struct ack_packet tmp = {0};
    tmp.type=ACKNOWLEDGE;
    return tmp;
}

struct ack_packet create_nack() {
    struct ack_packet tmp = {0};
    tmp.type=NACKNOWLEDGE;
    return tmp;
}
struct ack_packet create_tack() {
    struct ack_packet tmp = {0};
    tmp.type=TERMINATE;
    return tmp;
}

struct data_packet create_dack(uint32_t pn) {
    struct data_packet tmp = {0};
    tmp.pn=pn;
    return tmp;
}
